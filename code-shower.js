...
window.CodeState = {
    ...
    init: function() {
        ...
        codeContainers.forEach((container) => {
            ...
            fetch(filename)
                ...
                .then(codeContent => {
                    this.renderBlock(container, extension, codeContent);
                })
                .catch(error => {
    ...
    renderBlock: function(container, extension, content) {
        container.style.cssText = "font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.6; max-width: 800px; margin: 20px auto; padding: 0;";
        ...
        container.innerHTML = `
            <div class="code-block-inner" style="background-color: #1e1e1e; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
                <div class="code-header" style="display: flex; justify-content: space-between; align-items: center; background-color: #2d2d2d; padding: 8px 16px; color: #bcbcbc; font-size: 0.85rem; font-family: monospace; border-bottom: 1px solid #313244;">
                    <span>${extension}</span>
